  ...
  function witchDataTable(id,options) {
        ...
        _self.addMenu=function() {
            _dom.find('tbody > tr').unbind().bind('contextmenu',function(e) {
                var o=$(this);
                $(options.rmenu.id).data('id', o.attr('id')).css({'left': e.pageX,'top': e.pageY,'display':'block'}).focus().unbind().bind('blur',function() {$(this).css('display','none');});
                $('.trc').removeClass('trc');
                o.addClass('trc');
                //alert(o.attr('id'));
                return false;
            ...
            if(checkValidate(o.index(), input.val(),o)) {
							if(o.data('d') !== input.val()) {
								$.ajax({
									 ...
									 data: _dom.find('th:eq(' +  o.index() + ')').data('edit') + "=" + input.val() + "&id=" + _dom.find('th:eq(' +  o.index() + ')').data('edit') ,
									 success: function(msg) {
										 var oo=$(this);
