...
function witchDataTableOrder(id, options) {
	...
	function init() {
		    ...
		    thead += tbody;
				...
				_dom.find('th').each(function (i) {//添加表单验证规则
					if (theadOp[i]) {
						$(this).data('validate', theadOp[i].validate).data('edit', theadOp[i].edit).data('ajaxUrl',theadOp[i].ajaxUrl).data('cfun',theadOp[i].cfun).data('data',theadOp[i].jsonData);
					}
				});
				_self.addMenu();
				_self.editInput();
	...
	function editTd(o) {
		...
		if(thindex.data('ajaxUrl') || thindex.data('jsonData')) {
			console.log('789');
			input.witchAuto({
				class: 'f-auto',
