 ...
 */
  function witchDataTable(id,options) {
        ...
        _self.addMenu=function() {
            _dom.find('tr').unbind().bind('contextmenu',function(e) {
                var o=$(this);
                $(options.rmenu.id).attr('id', o.attr('id')).css({'left': e.pageX,'top': e.pageY,'display':'block'}).focus().unbind().bind('blur',function() {$(this).css('display','none');});
                $('.trc').removeClass('trc');
                o.addClass('trc');
                return false;
            }).bind('click',function() {
