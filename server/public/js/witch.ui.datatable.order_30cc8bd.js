...
function witchDataTableOrder(id, options) {
	...
	function init() {
		    ...
		    thead += tbody;
				...
				_dom.find('th').each(function (i) {//添加表单验证规则
					if (theadOp[i]) {
						$(this).data('validate', theadOp[i].validate).data('edit', theadOp[i].edit).data('ajaxUrl',theadOp[i].ajaxUrl).data('cfun',theadOp[i].cfun);
					}
				});
				_self.addMenu();
				_self.editInput();
	...
	function editTd(o) {
		...
		//绑定失去焦点事件
		input.bind('blur', function () {setTimeout(function(o,oo){
