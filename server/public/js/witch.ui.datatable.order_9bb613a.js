...
function witchDataTableOrder(id, options) {
	...
	function init() {
		    ...
		    thead += tbody;
				...
				_dom.find('th').each(function (i) {//添加表单验证规则
					if (theadOp[i]) {
						$(this).data('validate', theadOp[i].validate).data('edit', theadOp[i].edit).data('ajaxUrl',theadOp[i].ajaxUrl).data('cfun',theadOp[i].cfun).data('gd',theadOp[i].jsonData);
					}
				});
				_self.addMenu();
				_self.editInput();
	...
	_self.addMenu = function () {
		_dom.find('tbody > tr').unbind().bind('contextmenu',function (e) {
			var o = $(this);
			$(options.rmenu.id).data('id', o.attr('id')).css({'left': e.pageX, 'top': e.pageY, 'display': 'block'}).focus().unbind().bind('blur', function () {
	...
	function editTd(o) {
		...
		if(thindex.data('ajaxUrl') || thindex.data('data')) {
			input.witchAuto({
				...
				jsonData: o.parent().find('.editdata').data('gd')
			});
			console.log("[[" + o.parent().find('.editdata').data('gd'));
		}
		//绑定失去焦点事件
		input.bind('blur', function () {setTimeout(function(){
